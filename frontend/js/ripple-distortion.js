// Vanilla-JS port of the RippleDistortion React component. The shader code
// (waveVertex, waveFragment, screenVertex, compositeFragment) and the wave
// physics/animation loop below are unchanged from the original - only the
// React wrapper (useEffect/useRef/props-as-state, JSX return) was stripped
// and replaced with a plain mount/destroy pattern.
import { Renderer, Program, Mesh, Geometry, Triangle, Texture, RenderTarget } from "../vendor/ogl.mjs";
...
export function createRippleDistortion(mountElement, options = {}) {
  const opts = { ...DEFAULTS, ...options };
  const { src, rings, strength, swirl, dispersion, glint, tint, tintAmount, grayscale, highlightColor, quality } = opts;

  const mount = mountElement;
  mount.classList.add("ripple-distortion");

  // Mutable per-frame config, mirrors the React version's configRef.
  const cfg = {
    brushSize: opts.brushSize,
    spread: opts.spread,
    fade: opts.fade,
    spacing: opts.spacing,
    clickStrength: opts.clickStrength,
